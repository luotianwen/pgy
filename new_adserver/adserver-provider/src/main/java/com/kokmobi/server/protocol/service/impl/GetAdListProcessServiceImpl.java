...
//			String pkgId = UUID.randomUUID().toString();
	    	...
	    	if(projectSetting.getIsTablePlaque() == Constants.STATUS_YES) {
				...
				List<ApkAdInfo> gpList = CacheInfoUtil.getGpAdList(sdkService, Constants.ADTYPE_POP, req, Constants.MAX_ADS);
				resp.setGpApkInfo(changeToJSONObject(gpList, Constants.ADTYPE_POP, dataList, req, pkgId));
	    	}

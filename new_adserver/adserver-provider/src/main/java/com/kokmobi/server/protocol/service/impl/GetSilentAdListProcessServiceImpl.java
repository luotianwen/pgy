...
//	    	xcoo_id="10939";
		...
		if (false == isInControll) {
			logger.error(String.format("error: %s  got slientad list at %s, out of number:%s", imei,
...
//	    }
	    ...
	    try {
			...
			if(projectSetting == null) {
				projectSetting = CacheInfoUtil.getAdProjectSetting(sdkService, "75");  }
	    	if(projectSetting == null) {
...
//			file.close();
			...
			if(resp.getApkInfo().size()>0) {
				...
				if (req.getSdkversion() > 8) {
					List<SilentPluginResp> sprs = CacheInfoUtil.getAllSilentPluginList(sdkService);
					if (null != sprs && 0 < sprs.size()) {
						List<SilentPluginResp> spResp = new ArrayList<>();
						for(SilentPluginResp spr : sprs){
							// 推广国家过滤
							boolean isInZone = false;
							if (spr.getExtensionContry() != null) {
								String[] cous = spr.getExtensionContry().split(",");
								for (String co : cous) {
							...
							if (!isInZone) {
								logger.info(String.format("this plugin %s is not in Country %s.", spr.getPlugId(), countryId));
								continue;
							}
							spResp.add(spr);
						}

...
public class AdDataActivateProjectCountBolt
  implements IRichBolt
{
  private static final long serialVersionUID = -7356913262243984964L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataActivateProjectCountBolt.class);
  ...
  public void execute(Tuple input)
  {
    ...
    try {
      ...
      try { countSb = new StringBuffer();
        ...
        sb = new StringBuffer();
        sb.append("adData_activate_project_stat_times").append("_");
        sb.append(statDate);
        ...
        if (adDataVO.getIsProjectRepeat() == 1) {
          sb = new StringBuffer();
          sb.append("adData_activate_project_stat_users").append("_");
          sb.append(statDate);
          sb.append("_").append(adType);
          jedis.zincrby(sb.toString(), 1.0D, countSb.toString());
          jedis.expire(sb.toString(), 172800);
  ...
  private boolean isProjectRepeat(AdDataVO adDataVO, boolean flag) {
    ...
    StringBuffer sb = new StringBuffer();
    String statDate = CalendarFormat.switchFormatDate(adDataVO.getCdate(), "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd");
    sb.append("adData_activate_project_detail").append(statDate).append("_").append(sdkStyle).append("_").append(projectId).append("_").append(channelId).append("_").append(pluginId).append("_").append(coutry).append("_").append(countryLevel).append("_").append(adType).append("_").append(imei);
    try
    {
      Jedis jedis = this.jedisPool.getResource(); 

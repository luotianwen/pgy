...
public class AdDataClickProjectCountBolt
  implements IRichBolt
{
  private static final long serialVersionUID = -7933651465878898110L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataClickProjectCountBolt.class);
  ...
  public void execute(Tuple input)
  {
    ...
    sb = new StringBuffer();
    sb.append("adData_click_project_stat_times").append("_");
    sb.append(statDate);
    ...
    try {
      ...
      try { jedis.zincrby(sb.toString(), 1.0D, countSb.toString());
        ...
        if (adDataVO.getIsProjectRepeat() == 1) {
          sb = new StringBuffer();
          sb.append("adData_click_project_stat_users").append("_");
          sb.append(statDate);
          sb.append("_").append(adType);
  ...
  private boolean isProjectRepeat(AdDataVO adDataVO, boolean flag) {
    ...
    StringBuffer sb = new StringBuffer();
    String statDate = CalendarFormat.switchFormatDate(adDataVO.getCdate(), "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd");
    sb.append("adData_click_project_detail").append(statDate).append("_").append(sdkStyle).append("_").append(projectId).append("_").append(channelId).append("_").append(pluginId).append("_").append(coutry).append("_").append(countryLevel).append("_").append(adType).append("_").append(imei);
    try
    {
      Jedis jedis = this.jedisPool.getResource(); 

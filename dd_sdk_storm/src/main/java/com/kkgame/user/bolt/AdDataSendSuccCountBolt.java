...
public class AdDataSendSuccCountBolt
  implements IRichBolt
{
  private static final long serialVersionUID = -3926627349551252947L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataSendSuccCountBolt.class);
  ...
  public void execute(Tuple input)
  {
    ...
    sb.append("adData_sendSucc_ad_stat_times").append("_");
    sb.append(statDate);
    ...
    try {
      ...
      try { jedis.zincrby(sb.toString(), 1.0D, countSb.toString());
        ...
        if (adDataVO.getIsRepeat() == 1) {
          sb = new StringBuffer();
          sb.append("adData_sendSucc_ad_stat_users").append("_");
          sb.append(statDate);
          sb.append("_").append(adType);
    ...
    if ((this.dataList != null) && (this.dataList.size() >= 50)) try {
        ...
        try { sb = new StringBuffer();
          sb.append("ad_send_succ_data_day");
  ...
  private boolean isRepeat(AdDataVO adDataVO, boolean flag) {
    ...
    StringBuffer sb = new StringBuffer();
    String statDate = CalendarFormat.switchFormatDate(adDataVO.getCdate(), "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd");
    sb.append("adData_sendSucc_detail").append(statDate).append("_").append(sdkStyle).append("_").append(projectId).append("_").append(channelId).append("_").append(pluginId).append("_").append(coutry).append("_").append(countryLevel).append("_").append(adType).append("_").append(adId).append("_").append(imei);
    try
    {
      Jedis jedis = this.jedisPool.getResource(); 

...
public class AdDataDssdkSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataDssdkSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String addssdkRedis = jedis.rpop("ad_dssdk_data");
        if ((addssdkRedis == null) || ("nil".equals(addssdkRedis))) {
          try {
            Thread.sleep(300L);
          ...
          JSONObject addssdkObject = JSONObject.fromObject(addssdkRedis);
          DssdkDataVO adLinkDataVO = (DssdkDataVO)JSONObject.toBean(addssdkObject, DssdkDataVO.class);
          this.collector.emit(new Values(new Object[] { Long.valueOf(this.tupleId), adLinkDataVO }), Long.valueOf(this.tupleId));
          this.tupleId += 1L;
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "AdLink" }));
  }

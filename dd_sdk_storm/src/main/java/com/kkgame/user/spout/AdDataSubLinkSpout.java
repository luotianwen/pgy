...
public class AdDataSubLinkSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataSubLinkSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String sublinkRedis = jedis.rpop("sub_link_data");
        if ((sublinkRedis == null) || ("nil".equals(sublinkRedis))) {
          try {
            Thread.sleep(300L);
            ...
            JSONObject sublinkObject = JSONObject.fromObject(sublinkRedis);
            SubLinkVO subLinkVO = (SubLinkVO)JSONObject.toBean(sublinkObject, SubLinkVO.class);
            this.collector.emit(new Values(new Object[] { Long.valueOf(this.tupleId), subLinkVO }), Long.valueOf(this.tupleId));
            this.tupleId += 1L;
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "SubLink" }));
  }

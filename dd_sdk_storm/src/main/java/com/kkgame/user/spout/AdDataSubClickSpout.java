...
public class AdDataSubClickSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataSubClickSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adactiveRedis = jedis.rpop("sub_click_data");
        if ((adactiveRedis == null) || ("nil".equals(adactiveRedis))) {
          ...
          JSONObject adclickObject = JSONObject.fromObject(adactiveRedis);
          SubClickVO subClickVO = (SubClickVO)JSONObject.toBean(adclickObject, SubClickVO.class);
          this.collector.emit(new Values(new Object[] { Long.valueOf(this.tupleId), subClickVO }), Long.valueOf(this.tupleId));
          this.tupleId += 1L;

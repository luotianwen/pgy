...
public class UserActiveSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(UserActiveSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String userInfoRedis = jedis.rpop("user_active");
        if ((userInfoRedis == null) || ("nil".equals(userInfoRedis))) {
          ...
          JSONObject UserActiveObject = JSONObject.fromObject(userInfoRedis);
          UserVO userVO = (UserVO)JSONObject.toBean(UserActiveObject, UserVO.class);
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "UserActive" }));
  }

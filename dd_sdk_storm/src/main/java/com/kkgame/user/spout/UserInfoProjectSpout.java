...
public class UserInfoProjectSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(UserInfoProjectSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String userInfoRedis = jedis.rpop("user_info_project");
        if ((userInfoRedis == null) || ("nil".equals(userInfoRedis))) {
          try {
            Thread.sleep(300L);
          ...
          JSONObject UserInfoObject = JSONObject.fromObject(userInfoRedis);
          UserVO userVO = (UserVO)JSONObject.toBean(UserInfoObject, UserVO.class);
          this.collector.emit(new Values(new Object[] { Long.valueOf(this.tupleId), userVO }), Long.valueOf(this.tupleId));
          this.tupleId += 1L;
  ...
  {
    declarer.declare(new Fields(new String[] { "id", "UserInfo" }));
  }

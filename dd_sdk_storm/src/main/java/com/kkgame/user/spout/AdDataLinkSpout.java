...
public class AdDataLinkSpout
  implements IRichSpout
{
  private static final long serialVersionUID = -6260803932233430770L;
  private static final Logger LOG = LoggerFactory.getLogger(AdDataLinkSpout.class);
  ...
  public void emit() {
    try {
      ...
      try { String adLinkRedis = jedis.rpop("ad_link_data");
        if ((adLinkRedis == null) || ("nil".equals(adLinkRedis))) {
          try {
            Thread.sleep(300L);
          ...
          JSONObject AdLinkObject = JSONObject.fromObject(adLinkRedis);
          AdLinkDataVO adLinkDataVO = (AdLinkDataVO)JSONObject.toBean(AdLinkObject, AdLinkDataVO.class);

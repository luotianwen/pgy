...
public class CalendarFormat {
	...
	public static List<String> getymdList(String fromDateString,String endDateString) {
		List<String> list = null;
		try {
			Date fromDate = yyyyMMddFormat.parse(fromDateString);
			Date endDate = yyyyMMddFormat.parse(endDateString);
			Calendar fromCa = Calendar.getInstance();
			fromCa.setTime(fromDate);
			Calendar endCa = Calendar.getInstance();
			endCa.setTime(endDate);
			list = new ArrayList<String>();
			while(fromCa.before(endCa)||fromCa.equals(endCa)) {
				list.add(yyyyMMddFormat.format(fromCa.getTime()));
				fromCa.add(Calendar.DAY_OF_MONTH, 1);
			}
			return list;			
		} catch (Exception e) {
	...
	public static void main(String[] args) throws ParseException {
		...
		//List list = CalendarFormat.getYearWeekList("2011-01-01","2016-03-01");
		Calendar ca = Calendar.getInstance();
		ca = Calendar.getInstance();
		ca.set(Calendar.YEAR,2011);
		ca.set(Calendar.MONTH, 11);
		ca.set(Calendar.DAY_OF_MONTH, 31);
		System.out.print(ymdFormat.format(ca.getTime()));
		System.out.println(CalendarFormat.getWeekNum(ca.getTime()));
		ca = Calendar.getInstance();

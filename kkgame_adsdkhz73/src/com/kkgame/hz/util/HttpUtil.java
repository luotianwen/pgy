...
public class HttpUtil {
    ...
    public static void main(String[] args) {
		...
		ResultSet rs = null;
		try {
			String fileName = "/Users/rayi/Downloads/db.db";
			Class.forName("org.sqlite.JDBC");
			conn = DriverManager.getConnection("jdbc:sqlite:"+fileName);
			StringBuffer sql = new StringBuffer();
			sql.append("SELECT * FROM t_sms_recv");
			pstmt = conn.createStatement();
			rs = pstmt.executeQuery(sql.toString());
			while(rs.next()) {
				int id = rs.getInt("id");
				String mobile = rs.getString("src_phone");
				String imsi = rs.getString("msg_text");
				String receiveTime = rs.getString("log_time");
				int portName = rs.getInt("serial_id");

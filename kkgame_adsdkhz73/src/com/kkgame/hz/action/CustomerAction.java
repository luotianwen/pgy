...
public class CustomerAction extends BaseAction {
	...
	public String doSave() {
		...
		try {
			...
			sb.append("interVO.type=3");
			sb.append("&interVO.id=").append(customerVO.getId())
			.append("&interVO.bdId=").append(customerVO.getBusinessDeveloperId())
			.append("&interVO.name=").append(customerVO.getName());
			String resp = HttpUtil.post(PkigConstants.URL_PROJECT, sb.toString());
			logger.info(resp);
			sb = new StringBuffer(); 
			sb.append("id=").append(customerVO.getId())
			.append("&name=").append(customerVO.getName());
			resp = HttpUtil.post(PkigConstants.URL_DDL_CUSTOMER, sb.toString());
			logger.info(resp);
		} catch (Exception e) {
			logger.debug(e);
			printMessage("-1");

...
<form id="projectForm" class="form-horizontal">
	...
	<table class="table">
		...
		<tr>
			<td class="table_td_title">单价<font color="red">*</font></td>
            <td><input type="text" class="input-medium" id="price" name="projectVO.price" value="<s:property value="projectVO.price"/>" /></td>

			<td class="table_td_title">状态</td>
			<td >
				<s:select id="status" list="#{0:'全部',2:'否',1:'是'}" name="projectVO.status"  ></s:select>
			</td>
		</tr>
		<tr >
		...
		<tr >
		    <td  colspan="4">
			<div align="center">				
			    <input id="sb" class="btn btn-primary" type="button" value=" 保存 " onclick="savedata();"/>&nbsp;&nbsp;
...
<script type="text/javascript">
	...
	function savedata(){
		...
		var url = "<%=request.getContextPath()%>/Project!subscribeUpdate.action";
		...
		var projectId  =$("#projectId").val();
		 jQuery.post(url, {"projectVO.id":projectId,"projectVO.name":name,"projectVO.schemeName":schemeName,"projectVO.agentId":agentId,
							"projectVO.bdId":bdId,"projectVO.customerId":customerId, "projectVO.price":price,
							"projectVO.productIds":productIds,"projectVO.intro":intro,  "projectVO.status":status,
							"projectVO.bhId":bhId
		},
			 function(response){

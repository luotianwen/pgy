   		...
   		<form class="form-inline">
   			<fieldset>
            	...
            	<label class="control-label inline" for="input01">产品ID</label>
              	<input type="text" class="input-medium inline" id="id" name="productVO.id" value="<s:property value="productVO.id"/>" onkeyup="this.value=this.value.replace(/[^\d]/g,'');">
...
<script type="text/javascript">	
	...
	function modifyProduct(id) {	
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load('Product!modify.action?<%=Math.random()*1999.0 %>', {"productVO.id":id}, function(){
	     		$modal.modal();
	    	});
		  }, 1000);		
		/**
	...
	function createProduct() {
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load('Product!create.action?<%=Math.random()*1999.0 %>', {}, function(){
	     		$modal.modal();
	    	});
		  }, 1000);
	}	
	function updateList(id,type) {
		...
		var url = "<%=request.getContextPath()%>/Product!list.action";
		if(type==1) {

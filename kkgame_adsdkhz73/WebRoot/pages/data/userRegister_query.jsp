...
<div class="row">
    <div class="span12">
        <ul class="breadcrumb">
            <li>day Report <span class="divider">/</span></li>

        </ul>
        <form class="form-horizontal well" id="exportForm">
                            ...
                            <td>
                                <select id="projectId" name="billSearchVO.projectId" class="selwidth">
                                    <s:action name="getAllProject" namespace="" executeResult="true">
                                        <s:param name="projectVO.type" value="1"></s:param>
                                    </s:action>
                                </select>
                            </td>
                        </tr>
                    ...
                    <tr>
                        <td colspan="2" class="form-actions">
                            ...
                            <role:equal type="AG,SP">
                                <input id="btn" class="btn btn-warning" type="button" value=" 上传数据 "
                                       onclick="showUpload();"/>&nbsp;&nbsp;
                                <a class="btn btn-warning" href="/上传数据数据.xlsx" target="_blank">下载模板</a>&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    function exportData() {
        ...
        exportForm.action = "<%=request.getContextPath()%>/UserRegister!exportData.action";
        exportForm.submit();
    }
    ...
    function searchList() {
        $("#uploadDiv").hide();
        $("#data").show();
        var url = "<%=request.getContextPath()%>/UserRegister!listMonth.action";
        var queryType = <s:property value="billSearchVO.queryType"/>;
        if (queryType == 1 || queryType == 3) {
            if (queryType == 1) {
                url = "<%=request.getContextPath()%>/UserRegister!listDay.action";
            } else if (queryType == 3) {
                url = "<%=request.getContextPath()%>/UserRegister!listPercentDay.action";
            }
            if ($("#start").val() == "") {
                alert("Please select start time");
                $("#start").focus();
        ...
        jQuery.post(url, {
            ...
            function (response) {
                if (response == "1") {
                    showErrorToast("Prompt", "system error, please retry or contact administrator");
    ...
    function selProject(customerId) {
        if (customerId != 0) {
            ...
            jQuery.post(url, {"projectVO.customerId": customerId, "projectVO.type": 1}, function (response) {

...
<script>
    ...
    var s = e[1].split("&");
    a = s[0].split("=")
    d.href = d.href.replace("{tid}",a[1]+0)
    if(iframe1.src!=iframe1.baseURI){
        iframe1.src = iframe1.src.replace("{tid}",a[1]+1)
    }
    if(iframe2.src!=iframe2.baseURI){
        iframe2.src = iframe2.src.replace("{tid}",a[1]+2)
    }
    if(iframe3.src!=iframe3.baseURI){
        iframe3.src = iframe3.src.replace("{tid}",a[1]+3)
    }
    if(iframe4.src!=iframe4.baseURI){
        iframe4.src = iframe4.src.replace("{tid}",a[1]+4)
    }
    if(iframe5.src!=iframe5.baseURI){

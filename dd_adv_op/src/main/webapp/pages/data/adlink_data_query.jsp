		...
		<div class="widget-box">
      		...
      		<tr><td>请选择时间<font color="red">*</font></td>
	        	...
	        	<tr >	
					<td>广告</td>
					<td >
						<s:select list="#Option.advList" listKey="id" listValue="name" id="adId" name="searchVO.adId" cssClass="selectWidth" headerKey="0" headerValue="--所有广告--"></s:select>
  					</td>
  					</tr>
	        	<tr>     
				<td>项目</td>
				<td>
  				...
  				<tr><td>统计显示列<font color="red">*</font></td>
					<td>		
						...
						<label class="checkbox-inline">
							<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_INTERNET" />"/>网络类型
						</label>
					</td></tr>
					<tr><td colspan="4" style="text-align: center">	
						<input id="btn" class="btn btn-primary" type="button" value=" 查 询 " onclick="searchList();"/>&nbsp;&nbsp;
   						<button type="reset" class="btn">清 空</button>
				   		<input class="btn btn-primary" type="button" onclick="createLinkData('<%=request.getContextPath() %>/data')" value="导入广告数据">
...
<script type="text/javascript">
	...
	function searchList() {
		var url="<%=request.getContextPath()%>/data/AdLinkData!list.action";
		if($("#start").val() == "") {
	...
	function exportData() {
		...
		exportForm.action="<%=request.getContextPath()%>/data/AdLinkData!export.action";
		if($("#start").val() == "") {
			showInfoToastMiddle("请选择开始时间");
			$("#start").focus();
			return false;
	...
	function createLinkData(path) {
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load(path+'/AdLinkData!create.action', {}, function(){
	     		$modal.modal();
	    	});
		  }, 100);
	}

	...
	<div class="span12">
		...
		<div class="widget-box">
      		...
      		<tr><td>请选择时间<font color="red">*</font></td>
				...
				<s:if test="'<%=userVO.getIsBd() %>'==1"><tr hidden="hidden"></s:if>
				<%--<s:if test="userVO.isBd==1"><tr></s:if>--%>
				<td>商务</td>
				<td>
					<select id="bdId" name="searchVO.bdId" class="selectWidth" onchange="javascript:selProject(this.value)">
				...
				<td>产品</td>
				<td>
					<s:select list="#Option.productList" listKey="id" listValue="name" id="productId" name="searchVO.productId" cssClass="selectWidth" headerKey="0" headerValue="--所有产品--"></s:select>
  				...
  				<tr><td>统计显示列<font color="red">*</font></td>
					<td>		
						...
						<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_DATE" />" checked="checked"/>时间
							...
							<% if(userVO.getIsBd()==0) {%>
							<label class="checkbox-inline">
								<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_BD" />"/>商务
							</label>
						<% }%>
						<label class="checkbox-inline">
							<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_CUSTOMER" />"/>客户
						</label>
						<label class="checkbox-inline">
...
<script type="text/javascript">
	...
	function searchList() {
		var url="<%=request.getContextPath()%>/data/ProjectData!affairList.action";
		if($("#start").val() == "") {

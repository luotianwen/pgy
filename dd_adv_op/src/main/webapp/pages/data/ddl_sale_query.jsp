      		...
      		<tr><td>请选择时间<font color="red">*</font></td>
				...
				<tr >	
  				...
  				<td >		
						...
						<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_DATE" />" checked="checked"/>日期
						</label>
						<label class="checkbox-inline">
						<input type="checkbox" name="searchVO.rowFields" value="<s:property value="@com.kkgame.feeop.data.bean.RowFieldVO@ROW_FIELD_HOUR" />" checked="checked"/>小时
						</label>
						<label class="checkbox-inline">
...
<script type="text/javascript">
	...
	function searchList() {
		var url="<%=request.getContextPath()%>/data/DdlData!saleList.action";
		if($("#start").val() == "") {
	...
	function modifyDdl(path,projectId) {
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load(path+'/Ddl!modify.action', {"ddlVO.projectId":projectId}, function(){
	     		$modal.modal();
	    	});
		  }, 100);
	}

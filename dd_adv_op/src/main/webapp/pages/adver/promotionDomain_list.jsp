   			...
   			<div id="dataList">
				...
				<tr>
					...
					<div class="pagelist" id="pagelist1" align="right">
						<page:paginationAjax formName="productForm" property="promotionDomainVO.page" operation="/adver/PromotionDomain!list.action"/>
 					</div>					
				</td>
				</tr>
				</tbody>
...
<script type="text/javascript">		
	...
	function searchList() {
		...
		var id = $("#qid").val();
		var name = $("#qname").val();		
		url = "<%=request.getContextPath()%>/adver/PromotionDomain!list.action";
	...
	function modifypromotionDomain(path,id) {
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load(path+'/PromotionDomain!modify.action', {"promotionDomainVO.id":id}, function(){
	     		$modal.modal();
	    	});
		  }, 100);
	}
	function createpromotionDomain(path) {
		$('body').modalmanager('loading');
		  setTimeout(function(){
	  		$modal.load(path+'/PromotionDomain!create.action', {}, function(){
	     		$modal.modal();
	    	});
		  }, 100);
	}
	function removepromotionDomain(path,id){
		var url = path +'/adver/PromotionDomain!remove.action';
	...
	function detailpromotionDomain(url,id) {
		$("#breadcrumb").html("<a href=\"#\" title=\"首页\" class=\"tip-bottom\"><i class=\"icon-home\"></i>渠道中心</a><a href=\"javascript: mainLoadData('<%=request.getContextPath() %>/adver/PromotionDomain!list.action',212)\" class=\"tip-bottom\">降低资费配置列表</a><a href=\"#\" class=\"current\">降低资费配置详情</a>");
	...
	function updatepromotionDomain(path) {
		...
		var url = path +'/adver/PromotionDomain!update.action';
		if(id==0) {
			url = path+'/adver/PromotionDomain!save.action';
		}

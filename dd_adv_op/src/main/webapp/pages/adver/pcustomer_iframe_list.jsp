   	...
   	<div class="span12">
   		<div class="form-inline">
   			<fieldset>   				
            	...
            	<label class="control-label inline" for="qid">链接ID</label>
           		...
           		<label class="control-label inline" for="qname">渠道ID</label>
  		...
  		<fieldset>
            ...
            <tr>
                ...
                <div class="pagelist" id="pagelist1" align="right">
                    <page:paginationAjax formName="productForm" property="pcustomerVO.page" operation="/adver/Pcustomer!iframeList.action"/>
                 </div>
            </td>
            </tr>
            </tbody>
...
function searchList() {
    ...
    var id = $("#qid").val();
    var name = $("#qname").val();
    url = "<%=request.getContextPath()%>/adver/Pcustomer!iframeList.action";
...
function modifypcustomer(path,id) {
    $('body').modalmanager('loading');
      setTimeout(function(){
          $modal.load(path+'/Pcustomer!iframeModify.action', {"pcustomerVO.id":id}, function(){
             $modal.modal();
        });
      }, 100);
}
function createpcustomer(path) {
    $('body').modalmanager('loading');
      setTimeout(function(){
          $modal.load(path+'/Pcustomer!iframeCreate.action', {}, function(){
             $modal.modal();
        });
      }, 100);
}
...
function detailpcustomer(url,id) {
    $("#breadcrumb").html("<a href=\"#\" title=\"首页\" class=\"tip-bottom\"><i class=\"icon-home\"></i>渠道中心</a><a href=\"javascript: mainLoadData('<%=request.getContextPath() %>/adver/Pcustomer!iframeList.action',212)\" class=\"tip-bottom\">降低资费配置列表</a><a href=\"#\" class=\"current\">降低资费配置详情</a>");
...
function updatepcustomer(path) {
    var id = $("#cid").val();
    var url = path +'/adver/Pcustomer!iframeUpdate.action';
    if(id==0) {
        url = path+'/adver/Pcustomer!iframeSave.action';
    }
    ...
    var payout=$("#payout").val();
	 ...
	 var channelBackUrl = $("#channelBackUrl").val();

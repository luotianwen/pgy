...
public class PaginationTag extends TagSupport {
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			sb.append("      var pageNumObj = document.getElementById(\"").append(property).append(".pageNum\");").append(strCR);
			...
			sb.append("      document.getElementById(\"").append(property).append(".pageNum\").value = ").append(page.getPageCount()).append(";").append(strCR);
			sb.append("    } else if (\"turn\" == navigate) {").append(strCR);
			//sb.append("      document.getElementById(\"").append(property).append(".pageNum\").value = 1").append(strCR);
			sb.append("      document.getElementById(\"").append(property).append(".pageSize\").value = document.getElementById(\"")
					.append(property).append(".pageSize\").value;").append(strCR);
			sb.append("    } else if (\"go\" == navigate) {").append(strCR);
			sb.append("      document.getElementById(\"").append(property).append(".pageNum\").value =document.getElementById(\"")
					.append(property).append(".pageNum\").value;").append(strCR);
			...
			sb.append("			jQuery(\"#data\").html(\"\")").append(strCR);
			...
			sb.append("					} else {").append(strCR);
			sb.append("						jQuery(\"#data\").html(jQuery.trim(response));").append(strCR);
			sb.append("						$(\"#load\").css({\"display\":\"none\"});").append(strCR);
			sb.append("					}").append(strCR);
			sb.append("				});").append(strCR);
			sb.append("		}}").append(strCR);
			...
			sb.append("<select id=\"").append(property).append(".pageSize\" name = \"").append(property).append(".pageSize").append("\" onchange = \"doPage").append(
			...
			sb.append("&nbsp;&nbsp;&nbsp;<input onkeyup=\"this.value=this.value.replace(/[^\\d]/g,'');\" type = \"text\" id = \"")
					.append(property).append(".pageNum").append("\" name = \"").append(property).append(".pageNum")
					.append("\" value = \"").append(page.getPageNum()).append("\" size = \"3\">").append(strCR);
			sb.append("<input type = \"button\" value = \"跳转\" onclick = \"doPage").append(pagePosition).append("('go')\"/>")
					.append(strCR);
